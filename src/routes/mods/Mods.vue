  ...
  <InstanceContent>
    <section class="flex flex-row overflow-y-auto gap-4 h-full">
      <section class="flex flex-col grow">
        ...
        <LoadingComponent class="flex flex-col overflow-y-auto grow" :promise="() => searchMod()" ref="modList">
      ...
      <section class="flex flex-col grow rounded-lg bg-black/30 shadow-md shadow-black/40 overflow-y-auto min-w-[200px] max-w-[200px] mb-1">
        <LoadingComponent class="flex flex-col gap-3 p-4 h-full" :promise="() => getAllCategories().then(() => initFilter())" :execute-automation="true">
          <button type="button" v-for="category in selectedCategories" :key="category.id" @click="searchWithCategories(category)" class="flex flex-row text-left gap-2 rounded-sm border-2 border-gray-800 px-2 py-1 bg-gray-800">
