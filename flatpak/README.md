...
| File | Purpose |
| --- | --- |
| `io.github.fezlight.mine4ease.yml` | Flatpak build manifest (build-from-source) |
| `io.github.fezlight.mine4ease.metainfo.xml` | AppStream metadata (required, validated) |
| `io.github.fezlight.mine4ease.desktop` | Desktop entry |
...
```bash
cd flatpak
flatpak-builder --user --install --force-clean build-dir io.github.fezlight.mine4ease.yml
flatpak run io.github.fezlight.mine4ease
```

Validate the metadata before submitting:

```bash
appstreamcli validate flatpak/io.github.fezlight.mine4ease.metainfo.xml
desktop-file-validate flatpak/io.github.fezlight.mine4ease.desktop
```
...
2. Fork <https://github.com/flathub/flathub>, create a branch named
   `io.github.fezlight.mine4ease`, and add the manifest + metainfo + desktop file
