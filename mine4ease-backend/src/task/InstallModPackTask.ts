...
export function installModPack(modpackId: number, apiType: ApiType, gameVersion?: string, versionId?: number) {
    if (apiType === ApiType.CURSE && gameVersion) {
        return new InstallModPackCurseTask(modpackId, gameVersion);
    } else if (apiType === ApiType.FEEDTHEBEAST) {
        return new InstallModPackFeedTheBeastTask(modpackId, versionId);
    } else {
